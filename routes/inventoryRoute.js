 ...
 * The Router object is created using express.Router() and is used to define routes for inventory management operations. It allows us to handle HTTP requests related to inventory management, and to serve the appropriate views or process form submissions. By using a router, we can organize our routes in a modular way, making it easier to manage and maintain our code. The router will be exported at the end of the file so it can be used in other parts of the application.
 ...
 router() is a method provided by the Express framework that creates a new router object. This router object can be used to define routes for handling HTTP requests. By using a router, we can group related routes together and keep our code organized. In this case, we are creating a router for inventory management operations, which will allow us to define routes for displaying inventory views and processing form submissions related to inventory management.

 router.get() is a method used to define a route that responds to HTTP GET requests. It takes two arguments: the path for the route and a callback function that will be executed when a request is made to that path. In this file, we use router.get() to define routes for displaying inventory views, such as the inventory management view, inventory by classification view, and inventory detail view.

 router.post() is a method used to define a route that responds to HTTP POST requests. It also takes two arguments: the path for the route and a callback function that will be executed when a request is made to that path. In this file, we use router.post() to define routes for processing form submissions related to inventory management, such as adding classifications, adding inventory items, updating inventory items, and deleting inventory items.
 **********************/
...
/* Route to get inventory data in JSON format for a specific classification
csemotors.com/inv/getInventory/:classification_id
*/
...
// Route to process Delete Classification form submission
router.post(
    "/delete-classification",
    utilities.handleErrors(utilities.checkEmployeeOrAdmin),
